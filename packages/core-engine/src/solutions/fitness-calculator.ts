...
export interface FitnessBreakdown {
  ...
  maintainability: { score: number; weight: number };
...
export class FitnessCalculator {
  ...
  calculateFitness(solution: Solution, issue: DiagnosticIssue, context: SolutionContext): number {
    return this.calculateFitnessWithBreakdown(solution, issue, context).score;
  }

  calculateFitnessWithBreakdown(
    solution: Solution,
  ...
  private getPresetName(): string {
    for (const [name, weights] of Object.entries(WEIGHT_PRESETS)) {
      if (JSON.stringify(weights) === JSON.stringify(this.weights)) return name;
    }
    return 'custom';
  }
  ...
  private calculatePerformanceScore(solution: Solution): number {
    ...
    if (dbCallCount === 0) score += 15;
    else if (dbCallCount === 1) score += 25;
    else if (dbCallCount === 2) score += 10;
    else score -= (dbCallCount - 2) * 5;

    if (code.includes('include') || code.includes('populate') || code.includes('join')) score += 10;
    if (code.includes('select') || code.includes('projection')) score += 5;

    const loopCount = (code.match(/\bfor\s*\(|\bwhile\s*\(|\.forEach\(|\.map\(/g) || []).length;
    if (loopCount === 0) score += 5;
    else score -= loopCount * 3;

    if (code.includes('Promise.all') || code.includes('await Promise.all')) score += 8;
    if (code.includes('cache') || code.includes('memoize')) score += 10;
  ...
  private calculateComplexityScore(solution: Solution): number {
    ...
    if (solution.implementationTime < 30) score += 10;
    else if (solution.implementationTime > 120) score -= 20;
  ...
  private calculateCompatibilityScore(solution: Solution, context: SolutionContext): number {
    let score = 80;

    if (context?.existingPatterns?.includes(solution.type)) score += 20;
    if (solution.type === 'dataloader' && !context?.dependencies?.includes('dataloader')) score -= 15;
    if (solution.type === 'raw_join') score -= 10;

    return Math.max(0, Math.min(100, score));
  }

  rankSolutions(solutions: Solution[], issue: DiagnosticIssue, context: SolutionContext): Solution[] {
    return solutions
