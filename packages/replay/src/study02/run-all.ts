...
function buildSummaryMarkdown(allResults: { bad: BenchmarkResult; good: BenchmarkResult }[], metadata: any): string {
  ...
  lines.push(`## Overview`);
  lines.push('');
  lines.push(`Study 02 measures the prevalence and performance impact of synchronous (blocking) I/O operations in Node.js applications.`);
  lines.push('');
  lines.push(`The study combines:`);
  ...
  lines.push('');
  lines.push(`**For Step 1 (Repository Scan):**`);
  lines.push(`- Network access to clone public repositories`);
  lines.push(`- Sufficient disk space (~5-10GB for temporary clones)`);
  lines.push(`- Git installed`);
  lines.push('');
  lines.push(`**For Step 2 (Benchmarks):**`);
  ...
  lines.push('');
  lines.push(`To execute Study 02, use the original empirical-study repository:`);
  lines.push('');
  lines.push(`\`\`\`bash`);
  lines.push(`cd empirical-study/studies/02-blocking-io`);
  lines.push('');
  lines.push(`# Step 1: Repository scan`);
  lines.push(`npm run scan              # Clone + scan repos`);
  lines.push(`npm run scan:aggregate    # Aggregate scan results`);
  lines.push('');
  lines.push(`# Step 2: Performance benchmarks`);
  lines.push(`npm run bench:all         # Run all scenarios (bad + good)`);
  lines.push(`npm run bench:scenario -- --tc=1   # Run single scenario`);
  lines.push(`\`\`\``);
  lines.push('');
  lines.push(`---`);
  lines.push('');
  ...
  lines.push('');
  lines.push(`For full study execution:`);
  lines.push('');
  lines.push(`\`\`\`bash`);
  ...
  lines.push(`\`\`\``);
  lines.push('');
  lines.push(`Results will be saved to \`results/\` directory with both JSON and summary files.`);
